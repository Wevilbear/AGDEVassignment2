...
// the objects that in each grid is wrong compared to the actual grid its supposed to be in. 
//I believe the total number of objects is correct but saved in wrong grids
// 
/********************************************************************************
...
void CSpatialPartition::Update(void)
{
	for (int i = 0; i < xNumOfGrid; i++)
	{
		for (int j = 0; j < zNumOfGrid; j++)
		{
			...
			if (theGrid[i*zNumOfGrid + j].GetListOfObject().size() > 0 && !check)
			{
				//cout << theGrid[i*zNumOfGrid + j].GetListOfObject().size() << endl;
				theGrid[i*zNumOfGrid + j].SetMesh("REDGRIDMESH");
			}
		}
	}
	check = true;
...
void CSpatialPartition::Render(Vector3* theCameraPosition)
{
	...
	for (int i = 0; i<xNumOfGrid; i++)
	{
		...
		{
			modelStack.PushMatrix();
			modelStack.Translate(xGridSize*i - xGridSize / 2 + xGridSize - (xSize >> 1), 0.0f, zGridSize*j - zGridSize / 2 + zGridSize - (zSize >> 1));
			modelStack.PushMatrix();
...
float CSpatialPartition::CalculateDistanceSquare(Vector3* theCameraPosition, const int xIndex, const int zIndex)
{
	...
	float zDistance = (zGridSize*xIndex + (zGridSize >> 1) - (zSize >> 1)) - theCameraPosition->x;
