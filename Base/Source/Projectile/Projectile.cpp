...
void CProjectile::Update(double dt)
{
	...
	//Update AABB
	SetPAABB(Vector3(1.f, 1.f, 1.f), position);
}

void CProjectile::Update2(std::list<GenericEntity*> obj, std::list<CEnemy3D*>Enemy, double dt)
{
	if (m_bStatus == false)
		return;
	...
	for (auto &i : obj)
	{
		hit = EntityManager::GetInstance()->CheckOverlap(GetMinAABB(), GetMaxAABB(), i->GetMinAABB(), i->GetMaxAABB());
